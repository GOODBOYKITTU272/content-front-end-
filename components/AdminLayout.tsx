    ...
    <div className="flex h-screen bg-slate-50">
           ...
           {navItems.map((item) => (
             <button
                ...
                className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-md transition-colors ${
                    currentView === item.id || (currentView === 'USER_ADD' && item.id === 'USERS')
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-slate-900 z-40 pt-20 px-6 md:hidden">
            ...
            {navItems.map((item) => (
                <button
                    ...
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium transition-colors ${
                        currentView === item.id ? 'bg-red-600 text-white' : 'text-slate-400 hover:text-white'
                    }`}
                >
                    <item.icon className="w-5 h-5" />
                    <span>{item.label}</span>
           ...
           <button onClick={onLogout} className="text-red-400 flex items-center space-x-2 mt-8 px-4">
