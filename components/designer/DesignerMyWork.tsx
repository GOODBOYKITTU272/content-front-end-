...
const DesignerMyWork: React.FC<Props> = ({ user, projects, onSelectProject }) => {
    const myTasks = projects.filter(p => p.assigned_to_role === Role.DESIGNER && p.status !== 'DONE');
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        >
                            <div className="p-6 space-y-4">
                                ...
                                <div className="border-t-2 border-slate-100 pt-3">
                                    <button className="w-full bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 text-xs font-black uppercase border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
                                        {!project.delivery_date
                                            ? 'Set Delivery Date'
                                            : isDelivered
                                                ? 'View Details'
                ...
                {myTasks.length === 0 && (
                    <div className="col-span-full border-2 border-dashed border-black p-12 text-center bg-slate-50">
                        ...
                        <p className="text-slate-500 mt-2">No pending design tasks</p>
                    </div>
                )}
            </div>
        </div>
