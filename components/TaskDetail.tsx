...
const TaskDetail: React.FC<Props> = ({ project, currentUserRole, onClose, onUpdate }) => {
  ...
  const handleChange = (field: string, value: any) => {
    ...
    db.updateProjectData(project.id, { [field]: value });
  };

  const handleComplete = () => {
    if (confirm('Are you sure you want to submit this stage?')) {
      db.advanceWorkflow(project.id, comment);
      onUpdate();
      onClose();
    }
  };
  ...
  const handleReject = () => {
    ...
    db.rejectTask(project.id, rejectStage, comment || 'Rejected');
    onUpdate();
    onClose();
  };
                ...
                <div className="space-y-6">
                    {project.history.slice().reverse().map(event => (
                        <div key={event.id} className="flex space-x-4">
                            <div className={`mt-1 min-w-[16px] w-4 h-4 border-2 border-black ${
                                event.action === 'REJECTED' ? 'bg-[#FF4F4F]' : 
