...
const CineMyWork: React.FC<Props> = ({ user, projects, onSelectProject }) => {
    const myTasks = projects.filter(p => p.assigned_to_role === Role.CINE && p.status !== 'DONE');
                    ...
                    >
                        <div className="p-6 space-y-4">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                {project.shoot_date ? (
                                    <span className="text-[10px] font-bold text-green-600 uppercase">
                                        <CalendarIcon className="w-3 h-3 inline" /> Scheduled
                                    </span>
                                ) : (
                                    <span className="text-[10px] font-bold text-orange-600 uppercase">Pending Schedule</span>
                                )}
                            </div>
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                {project.video_link && (
                                    <div className="bg-blue-50 border-2 border-blue-400 p-2 mt-2">
                            ...
                            <div className="border-t-2 border-slate-100 pt-3">
                                <button className="w-full bg-[#0085FF] text-white px-4 py-2 text-xs font-black uppercase border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] group-hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all">
                                    {project.shoot_date ? (project.video_link ? 'View Details' : 'Upload Video') : 'Schedule Shoot'}
                                </button>
                            </div>
                        </div>
                    </div>
