...
import { Plus, Clock, Edit3, AlertCircle, FileText } from 'lucide-react';
    ...
    >
      <div className="space-y-8 animate-fade-in">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
             <div>
                <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-slate-900 mb-2 drop-shadow-sm">Writer Studio</h1>
                <p className="font-bold text-lg text-slate-500">Welcome back, {user.full_name}</p>
            </div>
             <button 
                onClick={() => setIsCreating(true)}
                className="bg-[#D946EF] text-white border-2 border-black px-6 py-4 font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center space-x-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-4">
                    {inReview.map(p => (
                        <div key={p.id} className="bg-white p-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] opacity-90">
                            <div className="flex justify-between items-start mb-4">
                ...
                <div className="space-y-4">
                    {inProduction.map(p => (
                        <div key={p.id} className="bg-slate-50 p-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            <div className="flex justify-between items-start mb-4">
                                <span className="text-xs font-black uppercase tracking-wider text-slate-400">{p.channel}</span>
                                 <span className="text-[10px] font-bold uppercase text-slate-500 bg-white border border-slate-300 px-2 py-0.5">{STAGE_LABELS[p.current_stage]}</span>
                            </div>
