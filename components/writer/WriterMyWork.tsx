...
const WriterMyWork: React.FC<Props> = ({ user, projects }) => {
    // Get writer's assigned work
    const myTasks = projects.filter(p =>
        p.assigned_to_role === Role.WRITER &&
        (p.status === TaskStatus.TODO || p.status === TaskStatus.IN_PROGRESS || p.status === TaskStatus.REJECTED)
    );
            ...
            <div className="grid gap-4">
                {myTasks.length === 0 ? (
                    ...
                    myTasks.map(task => (
                        <div
                            key={task.id}
                            className="bg-white p-4 sm:p-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all"
                        >
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        {task.status === TaskStatus.REJECTED && (
                                            <span className="px-3 py-1 bg-red-100 text-red-800 border-2 border-red-600 text-xs font-black uppercase">
                                                Rework Needed
                                            </span>
                                        )}
                                    </div>
