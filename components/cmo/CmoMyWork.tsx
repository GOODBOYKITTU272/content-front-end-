...
const CmoMyWork: React.FC<Props> = ({ user, projects, onReview }) => {
    // Get CMO's assigned work (projects awaiting L1 review)
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div>
                ...
                <p className="text-sm sm:text-base text-slate-600 font-medium">All scripts awaiting your review</p>
            </div>
            ...
            <div className="grid gap-4">
                {myTasks.length === 0 ? (
                    <div className="bg-slate-50 border-2 border-dashed border-slate-300 p-12 text-center">
                        ...
                        <p className="text-slate-500">No pending reviews at the moment</p>
                    </div>
                ) : (
                    myTasks.map(task => (
                        <div
                            ...
                            className="bg-white p-4 sm:p-6 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer"
                        >
