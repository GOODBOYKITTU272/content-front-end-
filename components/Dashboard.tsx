...
const Dashboard: React.FC<Props> = ({ user, projects, refreshData }) => {
  ...
  const filteredProjects = useMemo(() => {
    let list = projects;
    if (filter === 'assigned') {
      list = projects.filter(p => p.assigned_to_role === user.role && p.status !== TaskStatus.DONE);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {/* New Content Button Mockup for Header */}
        <div className="hidden md:block">
             <button className="bg-[#D946EF] text-white border-2 border-black px-6 py-3 font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all flex items-center space-x-2">
      ...
      {filter === 'assigned' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-[#0085FF] p-6 border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-white transition-transform hover:-translate-y-1">
                ...
                <div className="text-6xl font-black mb-2">{projects.filter(p => p.status !== TaskStatus.DONE).length}</div>
