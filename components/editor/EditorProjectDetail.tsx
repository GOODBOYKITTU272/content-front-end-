...
const EditorProjectDetail: React.FC<Props> = ({ project, onBack, onUpdate }) => {
    ...
    const handleSetDeliveryDate = () => {
        ...
        alert(`✅ Delivery date set for ${project.title} on ${deliveryDate}\n\nThis will be visible on calendars for Writer, CEO, CMO, Cinematographer, Designer, and Ops.`);
        onUpdate();
    };

    const handleUploadEditedVideo = () => {
        if (!editedVideoLink) {
            alert('Please enter the edited video link');
            return;
        }
        console.log(`Uploading edited video: ${editedVideoLink}`);
        alert(`✅ Edited video uploaded successfully!\n\nProject: ${project.title}\nLink: ${editedVideoLink}\n\n→ Designer will be automatically notified\n→ Project moving to THUMBNAIL_DESIGN stage`);
        onUpdate();
    };
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3">
                                    <input
                                        ...
                                        className="flex-1 p-4 border-2 border-black text-lgfont-medium focus:bg-yellow-50 focus:outline-none"
                                    />
                                    <button
