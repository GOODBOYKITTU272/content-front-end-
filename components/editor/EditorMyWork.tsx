...
const EditorMyWork: React.FC<Props> = ({ user, projects, onSelectProject }) => {
    const myTasks = projects.filter(p => p.assigned_to_role === Role.EDITOR && p.status !== 'DONE');
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                {!project.delivery_date ? (
                                    <span className="text-[10px] font-bold text-red-600 uppercase">Needs Date</span>
                                ) : project.edited_video_link ? (
                                    <span className="text-[10px] font-bold text-green-600 uppercase">
                                        <Film className="w-3 h-3 inline" /> Delivered
                                    </span>
                                ) : (
                                    <span className="text-[10px] font-bold text-orange-600 uppercase">In Progress</span>
                                )}
                            </div>
