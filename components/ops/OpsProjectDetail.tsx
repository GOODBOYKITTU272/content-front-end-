...
const OpsProjectDetail: React.FC<Props> = ({ project, onBack, onUpdate }) => {
    ...
    const handleSetPostDate = () => {
        console.log('Setting post date:', postDate);
        // In real app: update project.post_scheduled_date
        alert(`Post scheduled for ${format(new Date(postDate), 'MMM dd, yyyy')}`);
        onUpdate();
    };

    const handleAddLiveUrl = () => {
        console.log('Adding live URL:', liveUrl);
        // In real app: update project.data.live_url and set status to POSTED
        alert(`Live URL added: ${liveUrl}\nProject marked as POSTED ✅`);
        onUpdate();
    };
